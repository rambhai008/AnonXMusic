...
async def draw(_: app, m: t.Message):
    ...
    await m.reply_text(
            text=f"**ʜᴇʟʟᴏ {m.from_user.mention}**\n\n<b>sᴇʟᴇᴄᴛ ʏᴏᴜʀ ɪᴍᴀɢᴇ ɢᴇɴᴇʀᴀᴛᴏʀ ᴍᴏᴅᴇʟ</b>",
...
async def selectModel(_:app,query:t.CallbackQuery):
    ...
    if len(data) > 3:
        ...
        return
    modelId = int(data[1])
    await query.edit_message_text("<b>ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ, ɢᴇɴᴇʀᴀᴛɪɴɢ ʏᴏᴜʀ ɪᴍᴀɢᴇ.</b>")
    promptData = Database.get(auth_user,None)
    if promptData is None:
        return await query.edit_message_text("sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ @papa_of_telegram !!.")
    ...
    images[-1] = t.InputMediaPhoto(img_url[-1],caption=f"<b>Your Prompt :</b>\n`{promptData['prompt']}`")
